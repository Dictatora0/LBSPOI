...
| 萨日娜   | 2023302101082   | 前端界面开发、地图组件集成、用户交互体验优化、文档撰写 |
...
- **POI查询**：
  ...
  - 按中心半径查询（地图交互）
  - 按是否有扩展信息查询
  - 组合查询（支持同时使用关键词搜索和筛选条件，例如搜索"东湖"并筛选"湖北省"）
- **标准化数据格式**：
  - JSON格式返回
  - HTTP状态码
  - 业务错误代码
...
| `GET`    | `/pois/search`      | 按名称关键词搜索POI，支持分页和筛选条件组合                    | API Key         | Query params: `q` (keyword), `province`, `category`, `level` 等 | `200 OK`       | `{ "items": [ { "id": 2, "name": "Park XYZ" } ], "total": 5 }` |
